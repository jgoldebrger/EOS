...
export async function waitForOrgLanding(page: Page, orgSlug = E2E_ORG_SLUG): Promise<void> {
  await page.waitForURL(new RegExp(`/org/${orgSlug}(/|$)`), { timeout: 45_000 });
}
...
export async function signInAsAdmin(page: Page): Promise<void> {
  ...
  await page.waitForURL(/\/(org|auth)(\/|$)/, { timeout: 15_000 });
  if (page.url().includes("/auth")) {
    await submitSignIn(page, E2E_ADMIN_EMAIL, E2E_ADMIN_PASSWORD);
  }
...
export async function signInAsViewer(page: Page): Promise<void> {
  ...
  await page.waitForURL(/\/(org|auth)(\/|$)/, { timeout: 15_000 });
  if (page.url().includes("/auth")) {
    await submitSignIn(page, E2E_VIEWER_EMAIL, E2E_VIEWER_PASSWORD);
  }
